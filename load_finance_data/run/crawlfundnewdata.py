...
def crawl_new_data(code_str,crawlHandle,work_start_time):
    ...
    if load_data is None:
        fund_data.to_csv(target_file_name, columns=fund_data.columns.values.tolist(), index=True)
        currtime = datetime.datetime.now()
        hour, min, sec = caltimediff(work_start_time, currtime)
        ...
        load_data.sort_index(inplace=True)
        load_data.to_csv(target_file_name, columns=load_data.columns.values.tolist(), index=True)
        currtime = datetime.datetime.now()
        hour, min, sec = caltimediff(work_start_time, currtime)
